...
paq({
    ...
    'lewis6991/impatient.nvim',
    {
        'luafun/luafun',
        run = function ()
            return vim.cmd(('!cd ' .. (paqs_path .. '/start/luafun && mkdir -p lua && mv fun.lua lua')))
        end
    },
    ...
    'dccmx/vim-lemon-syntax',
    {
        ...
        run = function ()
            return vim.cmd('TSInstall all | TSUpdate')
        end
    },
...
do
    ...
    jv_tabline = function ()
        ...
        for i = 1, num_tabs do
            ...
            ret = (ret .. ("%" .. (tostring(i) .. ("T " .. (tostring(i) .. (" " .. (tostring(cwd[# cwd]) .. "/ ")))))))
        end
        ret = (ret .. '%#TabLineFill#')
        if (num_tabs > 1) then
            ret = (ret .. '%=%999X ✖︎ ')
