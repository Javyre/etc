...
name: unslop
description: >-
  Remove AI tells from chat, prose, documentation, and code comments when AI
  generated the text or introduced the slop.
---

# Unslop

Remove AI patterns while preserving meaning and intended tone.

## Process

1. Scan for the patterns below.
2. Rewrite the text. Preserve meaning and match the intended tone. Finish when
   the applicable patterns are resolved and the text reads naturally.

## Patterns to detect and fix

Rule numbers are stable IDs; gaps mark removed rules.
...
14. **Colon overuse.** Colons are fine before a list or example. Not as
    ...
    comparison framing. "Describing when the scheduler should fire works best
    as plain English." Same meaning, no crutch punctuation.
...
27. **Say what it does, not how it feels.** "the database stays close at hand",
    ...
    the mechanism or a number: "`.toSQL()` returns the exact string sent to the
    database", "a column rename fails the build". State what the reader needs
    to do or know. Cut claims that add no concrete instruction, fact, or number.
    If wording could appear unchanged in another project's docs, cut it unless
    it expresses a shared convention or constraint that changes the reader's
    action.
...
29. **Active voice.** Prefer it. Catch "is/are/was/were + past participle" and
    ...
    queries", "the file is parsed by the loader" becomes "the loader parses
    the file". Passive is fine only when the actor is unknown or doesn't matter.
